...
import { ListContractCommitmentsRequest$outboundSchema, ListContractCommitmentsResponseBody$inboundSchema, } from "../models/listcontractcommitmentsop.js";
 ...
 */
export function billingListContractCommitments(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => ListContractCommitmentsRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    }), { ctype: "application/jsonl" }), M.fail([400, 401, 403, 404, "4XX"]), M.fail("5XX"))(response, req);
    if (!result.ok) {
        return [result, { status: "complete", request: req, response }];
    }
    return [result, { status: "complete", request: req, response }];
}
//# sourceMappingURL=billingListContractCommitments.js.map
...
export const tool$projectsBatchRemoveProjectEnv = {
...
Delete multiple environment variables for a given project in a single batch operation.`,
    ...
    tool: async (client, args, ctx) => {
        const [result, apiCall] = await projectsBatchRemoveProjectEnv(client, args.request, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {

...
export const tool$projectsRequestRollback = {
...
Allows users to rollback to a deployment.`,
    ...
    tool: async (client, args, ctx) => {
        const [result, apiCall] = await projectsRequestRollback(client, args.request, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {

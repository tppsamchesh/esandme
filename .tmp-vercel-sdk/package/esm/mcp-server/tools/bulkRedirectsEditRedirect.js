...
export const tool$bulkRedirectsEditRedirect = {
...
Edits a single redirect identified by its source path. Stages a new change with the modified redirect and returns the alias for the new version in the response.`,
    ...
    tool: async (client, args, ctx) => {
        const [result, apiCall] = await bulkRedirectsEditRedirect(client, args.request, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {

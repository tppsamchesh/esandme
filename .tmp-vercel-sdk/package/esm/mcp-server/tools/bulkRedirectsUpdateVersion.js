/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import { bulkRedirectsUpdateVersion } from "../../funcs/bulkRedirectsUpdateVersion.js";
import { UpdateVersionRequest$inboundSchema } from "../../models/updateversionop.js";
import { formatResult } from "../tools.js";
const args = {
    request: UpdateVersionRequest$inboundSchema,
};
export const tool$bulkRedirectsUpdateVersion = {
    name: "bulk-redirects-update-version",
    description: `Promote a staging version to production or restore a previous production version.

Update a version by promoting staging to production or restoring a previous production version`,
    args,
    tool: async (client, args, ctx) => {
        const [result, apiCall] = await bulkRedirectsUpdateVersion(client, args.request, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {
                content: [{ type: "text", text: result.error.message }],
                isError: true,
            };
        }
        const value = result.value;
        return formatResult(value, apiCall);
    },
};
//# sourceMappingURL=bulkRedirectsUpdateVersion.js.map
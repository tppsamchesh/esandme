...
export const tool$bulkRedirectsGetVersions = {
...
Get the version history for a project's bulk redirects`,
    ...
    tool: async (client, args, ctx) => {
        const [result, apiCall] = await bulkRedirectsGetVersions(client, args.request, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {

...
export const tool$bulkRedirectsStageRedirects = {
...
Stages new redirects for a project and returns the new version.`,
    ...
    tool: async (client, args, ctx) => {
        const [result, apiCall] = await bulkRedirectsStageRedirects(client, args.request, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {

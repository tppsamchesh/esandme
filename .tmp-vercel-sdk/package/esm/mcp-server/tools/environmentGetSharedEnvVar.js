...
export const tool$environmentGetSharedEnvVar = {
...
Retrieve the decrypted value of a Shared Environment Variable by id.`,
    ...
    tool: async (client, args, ctx) => {
        const [result, apiCall] = await environmentGetSharedEnvVar(client, args.request, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {

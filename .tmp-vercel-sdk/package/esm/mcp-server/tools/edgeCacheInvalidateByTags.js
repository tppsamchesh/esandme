...
export const tool$edgeCacheInvalidateByTags = {
...
Marks a cache tag as stale, causing cache entries associated with that tag to be revalidated in the background on the next request.`,
    ...
    tool: async (client, args, ctx) => {
        const [result, apiCall] = await edgeCacheInvalidateByTags(client, args.request, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {

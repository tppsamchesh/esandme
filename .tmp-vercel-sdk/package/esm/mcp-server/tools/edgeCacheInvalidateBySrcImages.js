...
export const tool$edgeCacheInvalidateBySrcImages = {
...
Marks a source image as stale, causing its corresponding transformed images to be revalidated in the background on the next request.`,
    ...
    tool: async (client, args, ctx) => {
        const [result, apiCall] = await edgeCacheInvalidateBySrcImages(client, args.request, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {
